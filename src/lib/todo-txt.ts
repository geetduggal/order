...
export interface TodoItem {
  ...
  project?: string;
  /** `path:<vault-relative>` metadata. When present, this line was
   *  generated by the .md → todo.txt mirror and the source of truth
   *  lives at that file. Mutations on a mirrored line route to the
   *  .md; the next sync re-derives the line. */
  path?: string;
}
...
// path:VALUE — the mirror tag. Value is unquoted, non-whitespace; in
// practice it's a vault-relative path so we don't strip slashes. The
// mirror always writes it at the END of the line so a hand edit of
// the description doesn't accidentally shift it inside other tokens.
const PATH_TAG_RE = /\bpath:(\S+)/;
...
export function parseTodoTxt(body: string): TodoItem[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 6. `path:VALUE` — the mirror tag. Stripped from text so the
    //    rendered title doesn't show the file path.
    let path: string | undefined;
    const pthm = rest.match(PATH_TAG_RE);
    if (pthm) {
      path = pthm[1];
      rest = rest.replace(PATH_TAG_RE, "");
    }

    // 7. First `+project` token — left in `text` for the user's eyes
...
export function formatTodoItem(item: TodoItem): string {
  ...
  // path:<value> always at the end so a hand-edit of the description
  // doesn't accidentally land inside it. Mirrored lines round-trip
  // through this serializer; the user can edit the description part
  // freely as long as the path: tag stays put.
  const tail = item.path ? `path:${item.path}` : "";
  const tokens = [head.join(" "), item.text, tail].filter(Boolean);
...
/** Minimum shape a mirroring caller needs to describe an .md calendar
 *  event: the file's vault-relative path (used as the mirror tag), the
 *  display title, and the frontmatter keys that drive scheduling. */
export interface MirrorSource {
  ...
  allDay: boolean;
  completed?: boolean;
  /** Already-resolved Notable Folder name (the canonical NF the
   *  .md note belongs to, after stripping the `[[...]]` wikilink and
   *  inheriting from an Area/Category file if needed). The mirror
   *  kebab-cases this into the `+project` token. */
  folder?: string;
}

/** Compose a single mirrored TodoItem from an .md event. */
...
/** Rebuild the body so all mirrored lines are regenerated from the
 *  supplied .md sources; native lines (no `path:` field) survive
 *  untouched. Returns the new body, or null when no change is needed
 *  (lets callers skip a write to avoid a self-write reload cycle). */
export function syncMirroredBody(currentBody: string, sources: MirrorSource[]): string | null {
  const items = parseTodoTxt(currentBody);
  const native = items.filter((i) => !i.path);
  // Preserve the user's bare creation-date order on undated tasks at
  // the top; sort happens inside serializeTodoTxt for dated ones.
  const mirrored = sources.map(buildMirrorItem);
  const nextBody = serializeTodoTxt([...native, ...mirrored]);
  return nextBody === currentBody ? null : nextBody;
}
