...
// `[[wikilink]]`, so a `#`-led filename can't be linked; `$` sorts above the
// digits just the same and stays safe in links and filenames. They never combine
// on one name. Both are stripped before a name is matched, parsed, or displayed.
