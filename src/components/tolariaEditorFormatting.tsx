...
type OrderBasicTextStyle = 'bold' | 'italic' | 'strike' | 'code'
...
const TOLARIA_BASIC_TEXT_STYLE_ICONS = {
  ...
  code: Code2,
} satisfies Record<OrderBasicTextStyle, PhosphorIcon>

type OrderSelectedBlock = ReturnType<
  BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>['getTextCursorPosition']
>['block']

type OrderSelectedFileBlock = {
  type: string
  url: string
}
...
function getSelectedBlocksSafely(
  ...
  try {
    ...
    if (selectionBlocks?.length) {
      return selectionBlocks as OrderSelectedBlock[]
    }
  } catch {
  ...
  try {
    return [editor.getTextCursorPosition().block as OrderSelectedBlock]
  } catch {
    return []
  }
}
...
function getBasicTextStyleButtonState(
  basicTextStyle: OrderBasicTextStyle,
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
) {
...
function getOrderBlockTypeSelectOptions(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  firstSelectedBlock: OrderSelectedBlock,
) {
...
function OrderBasicTextStyleButton({
  basicTextStyle,
}: {
  basicTextStyle: OrderBasicTextStyle
}) {
...
function OrderBlockTypeSelect() {
  ...
  const selectedBlocks = useEditorState({
    editor,
    selector: ({ editor }): OrderSelectedBlock[] => getSelectedBlocksSafely(editor),
  })
  const firstSelectedBlock = selectedBlocks[0] ?? null
  const selectItems = useMemo(
    () => (
      firstSelectedBlock
        ? getOrderBlockTypeSelectOptions(editor, firstSelectedBlock)
        : []
    ),
    [editor, firstSelectedBlock],
  )
  const selectedItem = selectItems.find(
    (item): item is OrderBlockTypeSelectOption => item.isSelected,
  )
...
function OrderFileDownloadButton({ vaultPath }: { vaultPath?: string }) {
  const Components = useComponentsContext()!
  ...
  return items.flatMap((item) => {
    switch (String(item.key)) {
      case 'blockTypeSelect':
        return [<OrderBlockTypeSelect key={item.key} />]
      case 'boldStyleButton':
        return [<OrderBasicTextStyleButton basicTextStyle="bold" key={item.key} />]
      case 'italicStyleButton':
        return [<OrderBasicTextStyleButton basicTextStyle="italic" key={item.key} />]
      case 'strikeStyleButton':
        return [<OrderBasicTextStyleButton basicTextStyle="strike" key={item.key} />]
      case 'fileDownloadButton':
        return [<OrderFileDownloadButton key={item.key} vaultPath={vaultPath} />]
      default:
        return [item]
    }
  })
...
function insertInlineCodeButton(items: ReactElement[]) {
  ...
  return [
    ...
    <OrderBasicTextStyleButton basicTextStyle="code" key="codeStyleButton" />,
