...
window.__laputaTest = {
  ...
  async triggerMenuCommand(id: string) {
    ...
    if (!window.__laputaTest?.dispatchBrowserMenuCommand) {
      throw new Error('Order test bridge is missing dispatchBrowserMenuCommand')
    }
...
function showFatalRenderError(
  ...
  const existing = document.getElementById('order-fatal-render-error')
  const overlay = existing ?? document.createElement('pre')
  overlay.id = 'order-fatal-render-error'
